...
func (r *WebserverReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	variables.SetGlobalVariable(req.Name)
	// 2. Set the owner reference
	_, err := r.ReconcileDeployment(ctx, Webserver)
	if err != nil {
		return ctrl.Result{}, err
	}
	_, err = r.ReconcileService(ctx, Webserver)
	if err != nil {
		return ctrl.Result{}, err
	}

	_, err = r.ReconcileIngress(ctx, Webserver)
	if err != nil {
		return ctrl.Result{}, err
	}
